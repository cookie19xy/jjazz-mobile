...
pub struct SynthEngine { synthesizer: Synthesizer }

impl SynthEngine {
    pub fn from_file<P: AsRef<Path>>(sf2_path: P) -> Result<Self, String> {
        let mut file = File::open(sf2_path.as_ref()).map_err(|e| format!("Cannot open SoundFont: {}", e))?;
        let sf2 = Arc::new(SoundFont::new(&mut file).map_err(|e| format!("Failed to parse SoundFont: {}", e))?);
        let mut settings = SynthesizerSettings::new(44100);
        settings.enable_reverb_and_chorus = false;
        let synth = Synthesizer::new(&sf2, &settings).map_err(|e| format!("Failed to create synth: {}", e))?;
    ...
    pub fn reset(&mut self) { self.synthesizer.reset(); self.synthesizer.note_off_all(true); }
    ...
    pub fn render_tracks(&mut self, tracks: &[Phrase], bpm: f32) -> Vec<f32> {
        ...
        for track in tracks {
            for ne in &track.notes {
                let on = (ne.position * 60.0 / bpm * sr) as usize;
                let off = ((ne.position + ne.duration) * 60.0 / bpm * sr) as usize;
                events.push((on, track.channel, ne.pitch, ne.velocity, true));
                events.push((off, track.channel, ne.pitch, 0, false));
        ...
        for sample in 0..total_samples {
            while event_idx < events.len() && events[event_idx].0 == sample {
                ...
                if is_on { self.synthesizer.note_on(ch as i32, key as i32, vel as i32); }
                else { self.synthesizer.note_off(ch as i32, key as i32); }
                event_idx += 1;
            }
            let mut l = [0.0f32; 1]; let mut r = [0.0f32; 1];
            self.synthesizer.render(&mut l[..], &mut r[..]);
            output[sample * 2] = l[0]; output[sample * 2 + 1] = r[0];
        }
    ...
    pub fn render_chunk(&mut self, tracks: &[Phrase], bpm: f32) -> Vec<f32> {
        let sr = self.sample_rate() as f32;
        let mut events: Vec<(usize, u8, u8, u8, bool)> = Vec::new();
        let mut total_beats = 0.0f32;
        for track in tracks {
